...
class TauProcessor(processor.ProcessorABC):
    ...
    def __init__(self, min_reco_pf_pt=2, min_gen_tau_pt=20, decay_mode=0, min_gen_pion_pt = 2, min_jet_pt = 20, delta_r_matching = 0.4):
        self.min_reco_pf_pt_ = min_reco_pf_pt
        self.min_gen_tau_pt_ = min_gen_tau_pt
        self.min_gen_pion_pt_ = min_gen_pion_pt
        self.min_jet_pt_ = min_jet_pt
        self.decay_mode_ = decay_mode
    ...
    def get_gen_hadrons_from_taus(self, event_):
        ...
        hadrons_from_taus_ = hadrons_from_taus_[hadrons_from_taus_.pt > self.min_gen_pion_pt_]
    ...
    def get_gen_vis_taus(self, event_):
        gvt_ = event_.GenVisTau[
            ...
            (event_.GenVisTau.status == self.decay_mode_) &
...
#         return np.sqrt(deta**2 + dphi**2)
    ...
    def process(self, events):
        ...
        gen_vis_taus = gen_vis_taus[abs(gen_vis_taus.pion_lxy) < max_lxy]
...
#         print(f"[{dataset}] Tau children pdgIds for 0-pion events:\n{dau_from_taus.pdgId}")
        ...
        charged_reco_pf = self.get_charged_pf(leading_jet, self.min_reco_pf_pt_)
        ## count PF candidates per jet (axis=2), then sum per event (axis=1)
...
#         print(f"[{dataset}] GEN leading pions pts :\n{ak.to_list(sorted_gen_pions.pt[:100])}", flush=True)
        ...
        # Broadcast the tau pT to match the 0-or-1 shape of the pion, then insert it without corrupting the other data types
        ...
        leading_jet = self.get_leading_jet(jets)
        reco_pf =  self.get_charged_pf(leading_jet, self.min_reco_pf_pt_)
...
#         ecal_matched_pf_to_pions_masked = ak.mask(ecal_matched_pf_to_pions, dr_min < self.delta_r_matching_threshold_)
        ...
        delta_r_ecal = np.sqrt(deta_ecal**2 + dphi_ecal**2)

        # mask for ECAL matched
        ecal_is_matched = ak.any(delta_r_ecal < self.delta_r_matching_threshold_, axis=1)
        ...
        ### broadcast vars to [events, reco_pf, gen_pion]
        gen_lxy_broadcasted, _ = ak.broadcast_arrays(leading_gen_pion.lxy[:, None, :], delta_r_ecal)
        gen_dxy_broadcasted, _ = ak.broadcast_arrays(leading_gen_pion.dxy[:, None, :], delta_r_ecal)
        gen_phi_broadcasted, _ = ak.broadcast_arrays(leading_gen_pion.phi[:, None, :], delta_r_ecal)
        gen_phi_ecal_broadcasted, _ = ak.broadcast_arrays(leading_gen_pion.phi_at_ecal_pion[:, None, :], delta_r_ecal)
        gen_pt_broadcasted, _ = ak.broadcast_arrays(leading_gen_pion.pt[:, None, :], delta_r_ecal)
        gen_pdgid_broadcasted, _ = ak.broadcast_arrays(leading_gen_pion.pdgId[:, None, :], delta_r_ecal)
        gen_eta_broadcasted, _ = ak.broadcast_arrays(leading_gen_pion.eta[:, None, :], delta_r_ecal)
        phi_broadcasted, _ = ak.broadcast_arrays(flat_leading_reco_pf.phi[:, None, :], delta_r_ecal)
        phi_ecal_broadcasted, _ = ak.broadcast_arrays(flat_leading_reco_pf.phi_at_ecal[:, None, :], delta_r_ecal)
        pt_broadcasted, _ = ak.broadcast_arrays(flat_leading_reco_pf.pt[:, None, :], delta_r_ecal)
        dxy_broadcasted, _ = ak.broadcast_arrays(flat_leading_reco_pf.d0[:, None, :], delta_r_ecal)
...
#         flat_pass_gen_dxy = ak.flatten(leading_gen_pion[leading_gen_pion.eta_at_ecal_pion > -10].dxy)
        ...
        flat_ecal_matched_dxy = ak.fill_none(ak.flatten(ecal_matched_gen_pions.dxy), -99)

        flat_gen_pt = ak.fill_none(ak.flatten(leading_gen_pion.pt), -99)
        flat_matched_pt = ak.fill_none(ak.flatten(matched_gen_pions.pt), -99)
        flat_ecal_matched_pt = ak.fill_none(ak.flatten(ecal_matched_gen_pions.pt), -99)
...
#       },
      "Stau_300_100mm": {
        "files": {
          "root://cmseos.fnal.gov//store/group/lpcdisptau/displacedTaus/nanoprod/summary/Run3_Summer22_chs_AK4PFCands_v21/SMS-TStauStau_MStau-300_ctau-100mm_mLSP-1_TuneCP5_13p6TeV_madgraphMLM-pythia8/nano_0.root" : "Events",
